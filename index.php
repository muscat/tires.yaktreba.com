...
// TODO: брать из настроек из базы
...
function get_phones_on_timeslot_crlf(arr, timeslot) {
    let result="";
    for (item = 0; item < arr.length; item++ ) if ( arr[item]['timeslot'] == timeslot ) result = result + '\r\n' + arr[item]['phone'];
return result;
}

function get_phones_on_timeslot_comma(arr, timeslot) {
    let result="";
    for (item = 0; item < arr.length; item++ ) if ( arr[item]['timeslot'] == timeslot ) result = arr[item]['phone'] + ', ' + result;
    return result;
}
...
if ( time_to_refresh > 0 ) setInterval(function(){ refreshpage(); }, time_to_refresh );
...
$count=0; $days_shown=0; $check_today=0; $nextweek=0; $week_overflow=0;
    ...
    do {
        ...
        if ($day_of_week <= $working_days_of_week) {
            ...
            do {
                ...
                echo "<button class='time_button' id='timeslot-day-" . $this_day_epoch_begin . '-' . $timeslot . "' >" . date("H:i", $tmp_a + 3600 - ($is_dst * 3600)) . "</button>" . $eol;
